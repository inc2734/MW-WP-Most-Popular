...
class MW_WMP_track {
	...
	private $table = 'mw_wp_most_popular';
	...
	private function update_stats() {
		...
		if ( $this->post_id ) {
			...
			$raw_stats = $wpdb->get_var( $wpdb->prepare( "SELECT raw_stats FROM {$wpdb->prefix}{$this->table} WHERE post_id = '%d'", array( $this->post_id ) ) );
			$date = gmdate('Y-m-d');
			...
			if ( $raw_stats ) {
				...
				$wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}{$this->table} (post_id, last_updated, 1_day_stats, 7_day_stats, 30_day_stats, all_time_stats, raw_stats) VALUES ('%d', NOW(), '0', '0', '0', '0', '')", array( $this->post_id ) ) );
			}
			...
			$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}{$this->table} SET 1_day_stats = '{$count_1}', 7_day_stats = '{$count_7}', 30_day_stats = '{$count_30}', all_time_stats = all_time_stats + 1, raw_stats = '%s' WHERE post_id = '%d'", array( serialize( $raw_stats ), $this->post_id ) ) );
		}
	}
